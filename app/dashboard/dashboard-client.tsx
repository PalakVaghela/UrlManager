...
function filterBookmarks(bookmarks: Bookmark[], filter: Filter) {
  if (filter === "public") {
    return bookmarks.filter((b) => b.is_public);
  }
  if (filter === "private") {
    return bookmarks.filter((b) => !b.is_public);
  }
...
function FilterButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`text-left text-sm transition ${className} ${
        active
...
export default function DashboardClient({ bookmarks }: DashboardClientProps) {
  ...
  return (
    <div className="flex flex-1 flex-col md:flex-row">
      {/* Mobile: top pills */}
      <nav className="flex gap-2 overflow-x-auto border-b border-gray-200 p-4 md:hidden">
      ...
      {/* Desktop: sidebar */}
      <aside className="hidden w-52 shrink-0 border-r border-gray-200 p-6 md:block">
